...
public interface DataLoggerRepository extends JpaRepository<DataLogger, Long> {
    ...
    @Query(value = "SELECT * FROM dl_data_logger WHERE id = :id AND password = :password", nativeQuery = true)
    DataLogger findByIDAndPassword(@Param("id") Long id, @Param("password") String password);

    @Query(value = "SELECT * FROM dl_data_logger WHERE id = :id AND token = :token", nativeQuery = true)
    DataLogger findByIDAndToken(@Param("id") Long id, @Param("token") String password);
